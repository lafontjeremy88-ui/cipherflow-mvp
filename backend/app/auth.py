...
function MainApp({ token, userEmail, onLogout }) {
  ...
  // 🔥🔥🔥 CORRECTION CRITIQUE : GESTION INTELLIGENTE DU CONTENT-TYPE 🔥🔥🔥
  const authFetch = async (url, options = {}) => {
    const headers = {
      "Content-Type": "application/json", // Par défaut on veut du JSON
      ...options.headers,
      Authorization: `Bearer ${token}`,
    };

    // SI on envoie un fichier (FormData), on SUPPRIME le Content-Type.
    // Le navigateur va le remettre automatiquement avec le "boundary" nécessaire.
