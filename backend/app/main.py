...
class EmailHistoryItem(BaseModel): id: int; created_at: Optional[datetime] = None; sender_email: str; subject: str; summary: str; category: str; urgency: str; is_devis: bool; raw_email_text: str; suggested_response_text: str
...
async def upload_logo(req: LogoUploadRequest, db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    try:
        ...
        raise HTTPException(500, detail="Erreur traitement image")
...
async def analyze_file(file: UploadFile = File(...), db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    ...
    if data:
        db.add(FileAnalysis(filename=file.filename, file_type=str(data.get("type","")), sender=str(data.get("sender","")), extracted_date=str(data.get("date","")), amount=str(data.get("amount","")), summary=str(data.get("summary","")), owner_id=get_user_id(current_user)))
        db.commit()
    return data
