# CLAUDE CODE — CRÉATION PAGE MENTIONS LÉGALES
...
✅ Modifie uniquement les fichiers frontend liés à la page mentions légales

---

## ÉTAPE 1 — LOCALISE LA PAGE ACTUELLE (si elle existe)

```bash
grep -r "mentions\|legal\|legales" src/ --include="*.jsx" --include="*.tsx" -l 2>/dev/null
grep -r "mentions\|legal" public/ -l 2>/dev/null
```

---

## ÉTAPE 2 — CRÉE OU REMPLACE LE COMPOSANT

Crée ou remplace le fichier `src/pages/MentionsLegales.jsx` avec ce contenu :
      ...
      <footer style={{background:'white', borderTop:'1px solid #E2E8F0', padding:'32px 2rem'}}>
        <div style={{maxWidth:'860px', margin:'0 auto', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:'16px'}}>
          ...
          <div style={{display:'flex', gap:'20px'}}>
            <a href="/legal" style={{fontSize:'13px', color:'#2563EB', fontWeight:'500', textDecoration:'none'}}>Mentions légales</a>
...
## ÉTAPE 3 — VÉRIFIE ET AJOUTE LA ROUTE

Cherche dans `src/App.jsx` si la route `/legal` existe :

```bash
grep -r "legal\|mentions" src/App.jsx src/router* 2>/dev/null
```

Si la route n'existe pas, ajoute-la dans le router existant :
...
## ÉTAPE 4 — AJOUTE LE LIEN DANS LE FOOTER DE PRIVACY

Dans `src/pages/Privacy.jsx` (ou équivalent), ajoute le lien "Mentions légales" dans le footer si ce n'est pas déjà fait :

```jsx
// Dans la section footer de Privacy.jsx, ajouter :
<a href="/legal" style={{fontSize:'13px', color:'#94A3B8', textDecoration:'none'}}>
  Mentions légales
</a>
```
...
Vérifie :
- [ ] La page s'affiche à `/legal` sans erreur console
- [ ] Le sommaire latéral fonctionne (liens ancres)
- [ ] Les 3 cartes hébergeurs s'affichent en grille
- [ ] Le lien vers `/privacy` dans la section 5 fonctionne
- [ ] Les couleurs sont en light mode
- [ ] Le footer affiche les liens "Mentions légales / Confidentialité / Contact"
